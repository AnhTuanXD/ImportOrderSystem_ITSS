...
public class SiteStockView {
    private final AppContext context;
    private final User user;
    private final TableView<StockRecord> table = new TableView<>();
    private final HBox statsBar = new HBox(12);

    public SiteStockView(AppContext context, User user) {
    ...
    public Parent build() {
        ...
        table.getColumns().addAll(
                UiUtil.column("Mã hàng", StockRecord::getMerchandiseCode, 150),
                UiUtil.column("Số lượng tồn", r -> String.valueOf(r.getInStockQuantity()), 130),
                UiUtil.column("Đơn vị", StockRecord::getUnit, 100),
        ...
        add.setOnAction(e -> showStockDialog(null));
        edit.setOnAction(e -> {
            StockRecord selected = table.getSelectionModel().getSelectedItem();
            if (selected == null) {
                UiUtil.error("Vui lòng chọn một mặt hàng.");
                return;
            }
    ...
    private void refresh() {
        List<StockRecord> records = context.getStockRecords(user.getUsername());
        table.setItems(FXCollections.observableArrayList(records));
        updateStats(records);
    }

    private void updateStats(List<StockRecord> records) {
        int total = records.size();
        double avg = records.stream().mapToInt(StockRecord::getInStockQuantity).average().orElse(0);
    ...
    private void showStockDialog(StockRecord existing) {
        ...
        dialog.showAndWait().filter(Boolean::booleanValue).ifPresent(ignored -> {
            try {
                ...
                context.saveStockRecord(new StockRecord(
                        user.getUsername(), code.getText().trim(), qty, unit.getText().trim()));
                refresh();
    ...
    private void deleteSelected() {
        StockRecord record = table.getSelectionModel().getSelectedItem();
        if (record == null) {
            UiUtil.error("Vui lòng chọn một mặt hàng.");
            return;
        }
        if (UiUtil.confirm("Xác nhận xóa",
            ...
            try {
                context.deleteStockRecord(record.getSiteCode(), record.getMerchandiseCode());
                refresh();
